 ...
 * @author Deco
 */
class ModelResultadosFinaisCompleto extends ModelTableGateway 
{
	protected $tb = null; 

	public $primary_key = 'id_resultados_finais_completo';
	
	public $fields = array('id_resultados_finais_completo', 'id_login', 'maior_carreira', 'academica', 'empreendedora', 'gerencial', 'publica', 'politica', 'especialista', 'cp1', 'cp2', 'cp3', 'cp4', 'cp5', 'cp6', 'cp7', 'cp8', 'cp9', 'cp10', 'cp11', 'cp12', 'cp13', 'cp14', 'cp15', 'modificado', 'criado');
		
	public function __construct($tb, $adapter)
	{
		$this->tb = $tb;
		parent::__construct($this->tb->resultados_finais_completo, $adapter);
	}

	public function get($filter=array())
	{
		$alias = 'resultadosFinaisCompleto';
		...
		$qry->join(array('login'=>$this->tb->login),
		    "resultadosFinaisCompleto.id_login = login.id_login",
		    ['id_login'],
		    $qry::JOIN_LEFT);
